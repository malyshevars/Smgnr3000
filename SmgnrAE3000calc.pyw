...
def calculate_charcoal_mass(V_total, rate_g_per_l=25):
    """
    Рассчитывает массу БАУ (активированный уголь), г на 1 л.
    """
    return V_total * rate_g_per_l

def calculate_kau_mass(V_total, rate_g_per_l=15):
    """
    Рассчитывает массу КАУ (альтернативный уголь), 15 г на 1 л.
    """
    return V_total * rate_g_per_l
...
class DilutionCalculator(QWidget):
    ...
    def on_calculate(self):
        try:
            ...
            Mkau = calculate_kau_mass(Vfinal)

            self.result_water.setText(
                f"Добавить воды: {Vw:.3f} л  (итог: {Vfinal:.3f} л)"
            )
            self.result_charcoal.setText(
                f"БАУ для фильтрации: {Mchar:.1f} г"
            )
            self.result_kau.setText(
                f"КАУ для фильтрации: {Mkau:.1f} г"
            )
